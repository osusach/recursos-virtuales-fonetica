<template>
    <div class="flex flex-col gap-y-2 text-center bg-usach-aqua-300 rounded-lg p-5 font-usach-bebas-body">
        ...
        <div class="flex flex-col bg-usach-aqua-100 h-7 w-[100%] rounded-lg mb-4">
            <div class="flex h-7 rounded-lg items-center justify-center align-middle" :style="{ width: percentage + '%' }" :class="{
        ...
        <div v-for="(item, index) in correccion" class="flex flex-row place-content-between bg-usach-aqua-800 rounded-lg py-2 px-4 gap-x-6"
            :class="{
            ...
            <div class="flex flex-col text-center">
                ...
                <p v-if="dificultad < 1 && juego === 1" class=" font-usach-bebas-body text-lg"> Correcta: {{ item.answer }} sílabas</p>
                <p v-if="dificultad < 1 && juego === 2" class=" font-usach-bebas-body text-lg"> Correcta: </p>
                <p v-if="dificultad < 1 && juego === 3" class=" font-usach-bebas-body text-lg"> Correcta: {{ item.answer }} </p>
            </div>
        </div>
        <router-link to="/home" class="bg-usach-terra-700 mt-4 rounded-lg font-usach-bebas-body text-2xl p-3 hover:bg-usach-terra-800">
            <button>
                Terminar
            </button>
		</router-link>
