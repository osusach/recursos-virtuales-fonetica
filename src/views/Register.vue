<template>
    <form class="m-auto grid xl:grid-cols-1 gap-10 p-3 sm:p-10 w-full sm:w-[70%] lg:w-[40%]">
      <div class="flex flex-col text-5xl h-fit px-10 py-10 text-white text-center font-usach-helvetica-body bg-usach-aqua-800 rounded-lg [&>button]:">
        ...
        <Input Label="Usuario" @input="validateRegister" forLabel="user" type="text" placeholder="Usuario"/>
        <Input Label="Correo" @input="validateRegister" forLabel="email" type="text" placeholder="Correo"/>
        <select v-model="selectedFirst" @click="validateRegister" class="mb-4 font-usach-helvetica-body w-full h-11 rounded-lg text-usach-industrial-1000 bg-white border-white text-sm 
          ...
          <option value="sup" @click="selectedSec = ''">Educación Superior</option>
        </select>
        <select v-if="selectedFirst == 'basica'" @click="validateRegister" v-model="selectedSec" class="mb-4 font-usach-helvetica-body w-full h-11 rounded-lg bg-white text-usach-industrial-1000 border-white text-sm 
                                    focus:ring-usach-terra-700 focus:border-usach-terra-700" name="languages" id="lang">
          <option disabled value="">Grado</option>
          <option v-for="nivel in 8" :value="nivel">{{ nivel + '° Básico' }}</option>
        </select>
        <select v-if="selectedFirst == 'media'" @click="validateRegister" v-model="selectedSec" class="mb-4 font-usach-helvetica-body w-full h-11 rounded-lg bg-white text-usach-industrial-1000 border-white text-sm 
                                    focus:ring-usach-terra-700 focus:border-usach-terra-700" name="languages" id="lang">
          <option disabled value="">Grado</option>
          <option v-for="nivel in 4" :value="nivel">{{ nivel + '° Medio' }}</option>
        </select>
        <Input Label="Contraseña" @input="validateRegister" forLabel="password" type="password" placeholder="Contraseña"/>
        <Input Label="Confirme Contraseña" @input="validateRegister" forLabel="confirm_password" type="password" placeholder="Repetir contraseña"/>
